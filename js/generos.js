...
fetch(`https://api.themoviedb.org/3/genre/movie/list?api_key=c0945689b0a582e110971301d6ea8be2&language=es`)
    ...
    .then (function(datos){
        ...
        for (let i =0; i < datos.genres.length; i++ ){
            generos += `<a href="detail-genres.html?idGenero=${datos.genres[i].id}&genero=${datos.genres[i].name}&tipo=movie">${datos.genres[i].name} </a>`
        }
...
.then (function(datos){
    ...
    for (let i =0; i < datos.genres.length; i++ ){
        generos += `<a href="detailGeneroSerie.html?idGenero=${datos.genres[i].id}&genero=${datos.genres[i].name}&tipo=tv">${datos.genres[i].name} </a>`
    }
